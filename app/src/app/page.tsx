...
export default function HomePage() {
  ...
  const executiveCockpit = (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KPICard title="Unbanked Onboarded" value={kpiVal('Unbanked Onboarded', '2.4M')} status="neutral" />
        <KPICard title="Agent Network" value={kpiVal('Agent Network', '42K')} status="neutral" />
        <KPICard title="Rural Coverage" value={kpiVal('Rural Coverage', '78%')} status="neutral" />
        <KPICard title="Avg Remittance" value={kpiVal('Avg Remittance', '₱8,400')} status="neutral" />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 grid-cols-1">
        <Chart
          ...
          type="line"
          xKey="period"
          yKeys={[{ key: 'value', name: 'New Users (K)' }]}
          title="Financial Inclusion Growth (Monthly)"
        />
        <Chart
  ...
  const domainTab1 = (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <KPICard title="Agent Uptime" value={kpiVal('Agent Uptime', '94%')} />
        <KPICard title="Liquidity Issues" value={kpiVal('Liquidity Issues', '42/day')} />
        <KPICard title="Revenue/Agent" value={kpiVal('Revenue/Agent', '₱18K/mo')} />
      </div>
      <Chart
        ...
        title="Agent Distribution vs Population"
        height={400}
      />
    </div>
  );
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Chart
          ...
          title="Financial Inclusion Score by Province"
        />
        <ActionMemo
          persona={{ name: 'Marissa Gutierrez', role: 'VP Financial Inclusion' }}
          context={{}}
          onGenerate={async () => {
            const memos = [
              {
                ...
                actions: ['Deploy 500 agents in Mindanao underserved areas', 'Launch savings product linked to remittance', 'Partner with LGUs for conditional cash transfer distribution'],
              },
              {
  ...
  const askAiTab = (
    <div className="h-[600px]">
      <AskAI
        title="Ask AI"
        sampleQuestions={[
          'Which provinces have the lowest financial service coverage?',
          'Show correlation between agent density and remittance volume',
          'What is the cost-per-acquisition for unbanked customers?',
        ]}
        mode="sql"
  ...
  const tabs = [
    ...
    { id: 'domain-1', label: 'Agent Network', icon: '📈', content: domainTab1 },
    { id: 'domain-2', label: 'Impact', icon: '⚡', content: domainTab2 },
